...
#include "../lib/model/loot_generator.h"

using namespace std::literals;

SCENARIO("LostObject generation") {
  using model::LootGenerator;
  using TimeInterval = LootGenerator::Milliseconds;
      ...
      THEN("number of loot is proportional to loot difference") {
        for (unsigned loot = 0; loot < 10; ++loot) {
          for (unsigned looters = loot; looters < loot + 10; ++looters) {
            INFO("loot count: " << loot << ", looters: " << looters);
            REQUIRE(gen.Generate(TIME_INTERVAL, loot, looters) ==
                    looters - loot);
          }
        }
      }
